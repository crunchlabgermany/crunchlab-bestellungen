...
export const loader = async ({ request }) => {
  ...
  return { connection, posts, config: { configured: configuration.configured, redirectMatches: configuration.redirectMatches, environment: configuration.environment }, requiredScopes: TIKTOK_SCOPES };
};
...
export default function TikTok() {
  const { connection, posts, config, requiredScopes } = useLoaderData();
  ...
  return <s-page heading="Marketing · Social Media · TikTok"><AiTeamNav/>
    ...
    <s-section><div style={cardStyle}><h2>{connection ? "Verbunden" : "Nicht verbunden"}</h2>
      ...
      </div> : config.configured ? <a href="/app/ki-team/tiktok/oauth" target="_blank" rel="noopener noreferrer" style={{display:"inline-block",padding:"10px 14px",borderRadius:8,background:"#202223",color:"white",fontWeight:700,textDecoration:"none"}}>TikTok verbinden</a> : <s-banner tone="info">TikTok OAuth ist vorbereitet. Zuerst müssen die sicheren Umgebungsvariablen gesetzt und die Redirect-Weiterleitung geprüft werden.</s-banner>}
