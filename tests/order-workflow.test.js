...
test("Unbekannte Zutaten und Gewichtsabweichung blockieren die Freigabe",()=>{const order={paymentStatus:"PAID",lineItems:[{quantity:1}],ingredients:[{mappingStatus:"UNKNOWN",totalGrams:900,ingredient:null,name:"X"}]};const review=calculateReview(order);assert.equal(review.difference,-100);assert.ok(review.blockingReasons.length>=2)});
